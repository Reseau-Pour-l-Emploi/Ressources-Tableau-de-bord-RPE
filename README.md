...
Zone de contenu élargie (1360px), mise en page en deux colonnes : contenu principal (actualités en bandeaux colorés, webinaires en grille de cartes, newsletters, ressources en rangée défilante façon carrousel) et colonne latérale flottante (agenda façon calendrier avec pastille jour/mois, bloc contact). Au-delà de 5 éléments, un bouton "Afficher plus" apparaît (webinaires, newsletters) pour éviter qu'un bloc ne prenne toute la page. Site responsive (mobile/tablette) sur les trois pages.

## Console admin : Glossaire vs Accueil

La barre d'outils admin distingue désormais deux groupes d'onglets : **📚 Glossaire** (Propositions en attente / Mode édition) et **🏠 Accueil** (Contenu du site). Dans "Contenu du site", chaque bloc (agenda, actualités, newsletters, webinaires, ressources, liens de pied de page) permet d'ajouter, modifier et supprimer des éléments individuellement, sans toucher au code.
