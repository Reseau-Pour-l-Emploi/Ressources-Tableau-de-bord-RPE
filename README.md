...
## Bug corrigé : compteurs "en attente" non décrémentés pour les propositions en lot

Valider ou rejeter une proposition `groupe_lot`/`hierarchie_lot`/`groupe_suppression`/`hierarchie_suppression` ne décrémentait jamais `compteurs/{id}` des champs concernés (seules les propositions ajout/modification/suppression classiques, avec un `cible_id` unique, le faisaient). Corrigé via une fonction commune `getAffectedIds(div)` utilisée à la fois pour la validation et le rejet.

Un bouton **"🔄 Réparer les compteurs"** a été ajouté dans la barre d'outils admin : il recalcule `compteurs/{id}.en_attente` pour chaque champ à partir des propositions réellement `en_attente` en base (au lieu de se fier à l'historique incrément/décrément), ce qui corrige aussi les compteurs restés faussés par ce bug avant sa correction. À utiliser une fois après la mise à jour, puis en cas de doute.
