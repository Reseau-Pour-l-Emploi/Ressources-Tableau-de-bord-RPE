...
HIER_CHAINS = [
    ['Catégorie juridique', 'Catégorie juridique niv1', 'Catégorie juridique niv2', 'Catégorie juridique niv3'],
]
by_libelle_final = {r['libelle']: r for r in records}
for chain_labels in HIER_CHAINS:
    chain_ids = [by_libelle_final[l]['id'] for l in chain_labels if l in by_libelle_final]
    for i, rid in enumerate(chain_ids):
        rec = next(r for r in records if r['id'] == rid)
        rec['hierarchie'] = {
            'sup_id': chain_ids[i+1] if i+1 < len(chain_ids) else None,
            'inf_id': chain_ids[i-1] if i-1 >= 0 else None,
        }
